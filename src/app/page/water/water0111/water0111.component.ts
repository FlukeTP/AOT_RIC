...
@Component({
  selector: 'app-water0111',
  templateUrl: './water0111.component.html',
  styleUrls: ['./water0111.component.css']
})
export class Water0111Component implements OnInit {
  ...
  breadcrumb: any = [
    ...
    {
      label: "ปรับปรุงอัตราค่าภาระค่าประกันการใช้น้ำประปา",
      link: "#",
    },
  ...
  getExcel() {
    ...
    arrOfId.push("-");
    arrOfId.push("-");
    this.ajax.download(`${URL.EXPORT}/WATER0111/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  ...
  initDataTable = () => {
    ...
    this.dataTable.on('click', 'tbody tr button#edit', (e) => {
      var closestRow = $(e.target).closest('tr');
      var data = this.dataTable.row(closestRow).data();
      this.router.navigate(['water/water0111detail'], {
        queryParams: {
          id: data.waterInsuranceConfigId
        }
      });
    });
