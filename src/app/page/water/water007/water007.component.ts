...
const URL = {
  ...
  GET_ALL: "water007/get_all",
  SEND_TO_SAP: 'water007/sendToSAP',
}
@Component({
  selector: 'app-water007',
  templateUrl: './water007.component.html',
  styleUrls: ['./water007.component.css']
})
export class Water007Component implements OnInit {
  ...
  clickBtn() {
    this.dataTable.on('click', 'td > button#edit', (event) => {
      ...
      this.router.navigate(['/water/water007detail'], {
        queryParams: {
          waterCancelId: data.waterCancelId
        }
      })
    });
  ...
  getExcel() {
    ...
    this.ajax.download(`${URL.EXPORT}/WATER007/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
