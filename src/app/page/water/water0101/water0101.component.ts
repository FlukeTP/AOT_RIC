...
@Component({
  selector: 'app-water0101',
  templateUrl: './water0101.component.html',
  styleUrls: ['./water0101.component.css']
})
export class Water0101Component implements OnInit {
  ...
  getExcel() {
    ...
    arrOfId.push("-");
    arrOfId.push("-");
    this.ajax.download(`${URL.EXPORT}/WATER0101/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  ...
  initDataTable = () => {
    ...
    this.dataTable.on('click', 'tbody tr button#edit', (e) => {
      var closestRow = $(e.target).closest('tr');
      var data = this.dataTable.row(closestRow).data();
      this.router.navigate(['water/water0101detail'], {
        queryParams: {
          id: data.waterServiceConfigId
        }
      });
    });
