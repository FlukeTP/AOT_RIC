...
@Component({
  selector: 'app-water0102',
  templateUrl: './water0102.component.html',
  styleUrls: ['./water0102.component.css']
})
export class water0102Component implements OnInit {
  ...
  getExcel() {
    ...
    arrOfId.push("-");
    arrOfId.push("-");
    this.ajax.download(`${URL.EXPORT}/WATER0102/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  ...
  initDataTable = () => {
    ...
    this.dataTable = $('#datatable').DataTable({
      ...
      columns: [
        {
          data: 'airport', className: 'text-center'
        }, {
          data: 'modifiedDate',
          render(data) {
    ...
    this.dataTable.on('click', 'tbody tr button#edit', (e) => {
      var closestRow = $(e.target).closest('tr');
      var data = this.dataTable.row(closestRow).data();
      this.router.navigate(['water/water0102detail'], {
        queryParams: {
          id: data.waterMaintenanceConfigId
        }
      });
    });
