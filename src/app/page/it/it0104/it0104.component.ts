...
const URL = {
  ...
  GET_ALL: "it0104/get_all",
}
@Component({
  selector: 'app-it0104',
  templateUrl: './it0104.component.html',
  styleUrls: ['./it0104.component.css']
})
export class It0104Component implements OnInit {
  ...
  getExcel() {
    ...
    this.ajax.download(`${URL.EXPORT}/IT0104/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  // ================= data table ====================
  initDataTable = () => {
      ...
      columns: [
        {
          ...
          data: 'monthType', className: 'text-left'
        }, {
  ...
  clickBtn() {
    ...
    this.dataTable.on('click', 'td > button#edit', (event) => {
      const data = this.dataTable.row($(event.currentTarget).closest('tr')).data();
      this.router.navigate(['/it/it0104detail'], {
        queryParams: {
          itDedicatedCUTEConfigId: data.itDedicatedCUTEConfigId
        }
      })
    });
  }
