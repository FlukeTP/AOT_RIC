...
const URL = {
  ...
  GET_ALL: "it0102/get_all",
}
@Component({
  selector: 'app-it0102',
  templateUrl: './it0102.component.html',
  styleUrls: ['./it0102.component.css']
})
export class It0102Component implements OnInit {
  ...
  getExcel() {
    ...
    this.ajax.download(`${URL.EXPORT}/IT0102/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  // ================= data table ====================
  initDataTable = () => {
      ...
      columns: [
        {
          ...
          data: 'chargeRateType', className: 'text-left'
        }, {
  ...
  clickBtn() {
    ...
    this.dataTable.on('click', 'td > button#edit', (event) => {
      const data = this.dataTable.row($(event.currentTarget).closest('tr')).data();
      this.router.navigate(['/it/it0102detail'], {
        queryParams: {
          itOtherConfigId: data.itOtherConfigId
        }
      })
    });
  }
