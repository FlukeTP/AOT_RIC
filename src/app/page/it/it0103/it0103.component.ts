...
@Component({
  selector: 'app-it0103',
  templateUrl: './it0103.component.html',
  styleUrls: ['./it0103.component.css']
})
export class It0103Component implements OnInit {
  ...
  getExcel() {
    ...
    this.ajax.download(`${URL.EXPORT}/IT0103/${arrOfId.join(",")}`);
    this.commonService.unLoading();
  }
  // ================= data table ====================
  initDataTable = () => {
  ...
  clickBtn() {
    ...
    this.dataTable.on('click', 'td > button#edit', (event) => {
      ...
      this.router.navigate(['/it/it0103detail'], {
        queryParams: {
          itCUTETrainingConfigId: data.itCUTETrainingConfigId
        }
      })
    });
  }
