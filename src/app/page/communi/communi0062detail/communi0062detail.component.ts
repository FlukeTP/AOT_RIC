...
const URL = {
  SAVE: "communi0062/save",
  FIND_ID: "communi0062/find_id"
}
@Component({
...
export class Communi0062detailComponent implements OnInit {
  ...
  breadcrumb: any = [
    ...
    {
      label: "ปรับปรุงอัตราค่าภาระการใช้บริการหมวดสื่อสารอื่น ๆ ",
      link: "#",
    },
  ...
  // =================== initial setting ==================
  ngOnInit() {
    this.formGroup.get('commuLogoConfigId').patchValue(this.route.snapshot.queryParams['commuLogoConfigId']);
    if (Utils.isNotNull(this.formGroup.get('commuLogoConfigId').value)) {
      this.findById();
    }
  }
  // ================ action =====================
  ...
  findById() {
    ...
    this.ajax.doPost(URL.FIND_ID, this.formGroup.value).subscribe((res: ResponseData<any>) => {
      if (MessageService.MSG.SUCCESS == res.status) {
        this.formGroup.get('commuLogoConfigId').patchValue(res.data.commuLogoConfigId);
