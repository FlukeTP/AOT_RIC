...
@Component({
  selector: 'app-garbagedis0301',
  templateUrl: './garbagedis0301.component.html',
  styleUrls: ['./garbagedis0301.component.css']
})
export class Garbagedis0301Component implements OnInit {
  dataTable: any;
  dataList: any;
  remarkStr: string;
  ...
  initDataTable = (data: any[]) => {
      ...
      columns: [
        {
          ...
          data: 'trashType', className: 'text-center'
        },  {
          data: 'trashSize', 
          render(data) {
           return data + ' กก.'
    ...
    this.dataTable.on('click', 'tbody tr button#edit', (e) => {
      var closestRow = $(e.target).closest('tr');
      var data = this.dataTable.row(closestRow).data();
      this.onEdit(data.trashSizeServiceFeeId);
    });
    // history button
  ...
  onEdit(id: any) {
    console.log("IDD : ",id);
    this.router.navigate(['/garbagedis/garbagedis0301detail'], {
      queryParams: {
        id: id
      }
    });
